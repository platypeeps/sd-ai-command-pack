...
# ladder as `sd-ai-command-pack-full-check.sh` and the shared shell library's
# cache root -- override, the caller's working tree, then the hosting
# checkout. Under a fat install invoked from inside the repository the second
# rung returns exactly what the third one used to.
