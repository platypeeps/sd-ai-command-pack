...
Run from an ``sd-ai-command-pack`` source checkout::
    ...
    .venv/bin/python templates/scripts/sd-ai-command-pack-thin-resweep.py CONSUMER \\
        [--json] [--out PATH]
...
# Source-only operator tooling: it lives beside the shipped payload under
# templates/scripts/, so the pack checkout root is two levels up.
