...
- [ ] Running full-check in a repo whose base ref shares no history with HEAD
      warns and completes instead of exiting on the diff step.
- [ ] A non-base git error still fails the gate (no over-broad swallow).
- [ ] Test added and green on the CI matrix; shellcheck `-S warning` stays
