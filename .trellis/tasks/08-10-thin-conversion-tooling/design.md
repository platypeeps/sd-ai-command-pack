...
`blockers` is what stops conversion; `scheduled` is informational — the
pack references that live inside files this conversion deletes anyway.
Per contract C-A, `docs/SD_AI_COMMAND_PACK.md` hits land in `scheduled`,
which is the whole reason any consumer can reach `clear`.

Classification rule: a hit is `scheduled` when its file is in the
computed delete set, `blocked` otherwise. That makes the resweep depend
on the same plan builder `--thin` uses, so the two can never disagree
about what is being deleted — one enumeration, two readers.
