...
- [ ] All actions SHA-pinned with version comments; zizmor lane still green.
- [ ] A PR touching only `paths-ignore` paths can reach a mergeable state
      (verified by reasoning or a test PR).
- [ ] mypy lane runs in CI + `make check`; baseline committed if not clean.
- [ ] `dependabot.yml` present for pip + github-actions.
- [ ] ruff scope decision recorded (CONTRIBUTING or workflow comment) and
      applied.
