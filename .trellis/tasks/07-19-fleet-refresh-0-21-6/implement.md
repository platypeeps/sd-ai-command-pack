# Fleet Refresh 0.23.8 Implementation Plan

## Preparation

1. Verify `manifest.json`, tag `v0.23.8`, and the full-fleet candidate ledger.
...
2. Create `codex/sd-pack-0-23-8-refresh` from the synchronized default branch;
   the already-open Loadsmith PR may retain its original branch name.
...
7. Confirm the consumer returns clean to its synchronized default branch with
   installed version `0.23.8` and a passing post-merge audit.
