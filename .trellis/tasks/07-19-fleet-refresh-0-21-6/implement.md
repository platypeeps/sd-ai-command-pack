# Fleet Refresh 0.23.7 Implementation Plan

## Preparation

1. Verify `manifest.json`, tag `v0.23.7`, and the full-fleet candidate ledger.
...
2. Create `codex/sd-pack-0-23-7-refresh` from the synchronized default branch;
...
7. Confirm the consumer returns clean to its synchronized default branch with
   installed version `0.23.7` and a passing post-merge audit.
