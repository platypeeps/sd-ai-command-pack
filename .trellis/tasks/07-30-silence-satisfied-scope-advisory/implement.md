...
1. [ ] Extract `resolve_pr_body_scope_state` in
...
2. [ ] Rewrite `check_pr_body_scope` as a mapper over those tokens, preserving
       each existing message and exit status verbatim. Keep the
       `scoped_count -eq 0` early return above the helper call.
3. [ ] Replace the advisory branch in `main()` so it calls the helper and
...
4. [ ] Bound the advisory call: add `timeout: 10_000` and
...
5. [ ] `bash -n` the template, then `make sync` to regenerate the root
       `scripts/` mirror. Never edit the mirror by hand.
6. [ ] Add the seven advisory cases to `tests/test_review_scope.py` per
...
7. [ ] Add the preflight-level case to `tests/test_review_preflight.py`: scoped
...
8. [ ] Isolate every test from the developer's real `gh`, per the "Test isolation
...
9. [ ] Update `templates/docs/SD_AI_COMMAND_PACK.md:1871-1877` — the template,
       not the root mirror — to describe the new behavior and drop the "without
       contacting `gh` or a PR" promise.
10. [ ] `make sync` again after the doc edit.
11. [ ] Edit `manifest.json` to 0.56.5 by hand and add the matching top
...
12. [ ] `make check`, then `make release-prep` to validate the bump and refresh
