...
- [ ] A consumer can run one documented command that reports whether its
      installed pack version is behind the referenced upstream.
- [ ] Missing/unreachable upstream reference yields a soft note, not a
      failure; no existing gate changes exit code because of this feature.
- [ ] Installed guide documents the flow; docs twins byte-identical;
      review-preflight doc-path checker passes.
- [ ] If implemented in Python, shipped-scripts coverage floor maintained.
