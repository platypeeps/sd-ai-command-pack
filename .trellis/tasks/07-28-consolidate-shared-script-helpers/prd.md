...
- [ ] With `SD_AI_COMMAND_PACK_STATE_HOME` set, work-loop and fleet state resolve to the same root (today they diverge).
- [ ] Adding an eighth cache variable requires no shell-side edit.
- [ ] Session receipts and the KB write through the hardened atomic writer.
- [ ] No script constructs a git environment outside the lib.
- [ ] `make check` passes.
