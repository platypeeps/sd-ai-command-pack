...
- [ ] Introducing a deliberate syntax error into an `.opencode` JS file fails
      the lint lane (demonstrated, then reverted).
- [ ] The file set is derived from tracked files, not hardcoded.
- [ ] Lint lane stays green on the current tree; the parity test that pins the
