...
- [ ] `ruff check` passes with F811/F401 active on install.py + installer/*;
      per-file-ignore block for those files removed or reduced to a
      justified minimum.
- [ ] New test: an injected ENOSPC-raising write leaves no `.tmp` file in
      the destination directory (reproduces then locks the leak).
- [ ] 100% line+branch coverage on install.py + installer/* maintained;
      full suite green.
