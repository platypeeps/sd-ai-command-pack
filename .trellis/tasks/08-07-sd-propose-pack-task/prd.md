# Add sd-propose-pack-task to file or revise a pack task without disturbing a live session
...
The command is `sd-propose-pack-task`. It was originally scoped as
`sd-submit-pack-task`, which named only half the job: "submit" reads as filing
something new, and a user with a task to revise would not find it. "Propose"
covers both modes and is accurate about the boundary — the command always stops
at an open PR and never lands the change.
...
**Where the logic lives.** A skill under `.agents/skills/sd-propose-pack-task/`
