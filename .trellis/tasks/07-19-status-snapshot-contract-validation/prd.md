...
- [ ] Missing `status` is reported as an invalid work-loop snapshot.
- [ ] An unsupported `status` value is reported as invalid.
- [ ] Active or paused snapshots missing required renderer fields are reported
      as invalid instead of printing `None` values.
- [ ] Valid `none`, `invalid`, `unavailable`, active, and paused snapshots keep
      their current behavior.
- [ ] Focused status tests, twin checks, and the canonical pack check pass.
- [ ] A shipped payload change follows the normal version, changelog, release,
