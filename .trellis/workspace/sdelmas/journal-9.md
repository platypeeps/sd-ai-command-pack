...
## Session 438: Name the stage script in the bookkeeping-evidence remedy
<!-- trellis-session: v=2 fp=32a1c096e22f0ac0 -->

**Date**: 2026-08-28
**Task**: Name the stage script in the bookkeeping-evidence remedy
**Branch**: `fix/bookkeeping-evidence-plan-only-guidance`

### Summary

Fixed the --bookkeeping-evidence rejection sending controller callers to a flag that entry point does not accept (#576, PR #577, 0.71.63).

### Main Changes

- templates/scripts/sd-ai-command-pack-review-local.py: BOOKKEEPING_EVIDENCE_SHAPE now names sd-ai-command-pack-review-local.py --plan-only --json instead of saying "this command's own"; the comment above the constant records why.
- tests/test_review_stage.py: regression test asserting the remedy names the stage script and no longer self-refers.
- .trellis/spec/backend/error-handling.md: new Don't pattern -- a diagnostic a wrapper can forward is read outside the process that wrote it, so deixis resolves against the wrong referent.
- manifest.json 0.71.62 -> 0.71.63 with matching CHANGELOG heading; mirrors and fleet ledger regenerated by make release-prep.

### Git Commits

| Hash | Message |
|------|---------|
| `547a2a66001333ad1afe7def8d3526574d660884` | fix(review): name the stage script in the bookkeeping-evidence remedy |

### Testing

- [OK] make release-prep -- exit 0; version and changelog gates passed; candidate ledger current; full make check all lanes OK.
- [OK] New test alone: test_bookkeeping_evidence_guidance_names_the_stage_script ... ok.
- [OK] md5 over the four vendored copies of the stage script yields one distinct hash.
