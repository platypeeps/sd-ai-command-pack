...
2. Follow that skill exactly: run the deterministic local full-check gate with
   Prism/Gito disabled for this PR cycle, use the configured remote reviewer
   when available, perform automatic re-review after pushed fixes, inspect
   comments and CI, address or rebut feedback, commit and push appropriate
   fixes, and repeat until no new actionable comments remain.
