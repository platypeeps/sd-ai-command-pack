...
- The managed block in `.gitignore` is no longer generated by anything. The
  markers are kept only so `migrate-trellis` can find and remove the equivalent
  block in consumer repos at step 3-c; edit that section by hand like any other.
