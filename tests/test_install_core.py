...
class InstallCoreTests(InstallTestCase):
    ...
    def test_default_install_skips_plain_framework_dirs_without_trellis_markers(
        ...
        for platform in ("claude", "cursor", "gemini", "github", "opencode"):
            with self.subTest(platform=platform):
                self.assertIn(
                    f"active Trellis {platform} install not detected",
                    result.stdout,
                )
    ...
    def test_codex_visible_sd_skill_wrappers_reference_workflows(self) -> None:
        ...
        self.assertIn("the composite owns\n  `sd-review-pr` resolution", create_pr)
