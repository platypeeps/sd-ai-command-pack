...
focused on Trellis enrichment: continue, finish-work, PR review, full-check,
post-merge housekeeping, and refresh-specs wrapper workflows. The repository
and `sd` command namespace are intentionally broader than that initial scope,
so future skills, commands, scripts, docs, or rules may cover adjacent AI
workflow support that is not strictly Trellis-specific.
...
Codex exposes pack entry points as enabled skills named `sd-continue`,
`sd-finish-work`, `sd-full-check`, `sd-housekeeping`, `sd-review-pr`, and
`sd-refresh-specs`; type `/sd` in Codex command completion or invoke them
...
future `trellis update` runs. GitHub Copilot prompt files and OpenCode command
files use flat `sd-<command>` filenames so their slash-command completion lists
can surface them when you type `/sd`.
...
`/sd:review-pr`. GitHub Copilot prompt files, OpenCode command files, and Codex
skills expose the same entries as flat `sd-<command>` names, such as
`sd-review-pr`.
...
GitHub Copilot, OpenCode, and Codex) is optional but strongly recommended before
PR readiness.
...
The continue and finish-work commands are local namespace aliases for the
...
The review-pr command is local-review-first. It runs the full-check/Prism path
...
The refresh-specs command runs the existing Trellis `trellis-update-spec` skill
...
hand-editing generated output. If that refresh uses Repomix, the output map must
be `docs/repomix-map.md`. It then checks whether the repo already has an
