...
It saves no session.
The `opencode-json` reader attaches that review file with `--file`, runs as a private agent whose permission map denies writes, commands, fetches and reads outside the checkout, and takes its model from the registry entry, which must pin one.
