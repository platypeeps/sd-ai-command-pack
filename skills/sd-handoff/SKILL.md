...
`bin/sd-rules --for <path>`, which prints the live registry rows in scope for
that file (id, subject, teaching section) so the rules act while the author is
in the file rather than at review. Rows are **not** claimed — every
