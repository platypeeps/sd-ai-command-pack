...
def _parse_candidate_checks(
    item: Mapping[str, Any],
    label: str,
) -> tuple[tuple[str, ...], ...]:
    checks = item.get("candidateChecks")
    if not isinstance(checks, list) or not checks:
        raise FleetConfigError(f"{label} must list candidateChecks")
    parsed: list[tuple[str, ...]] = []
    for check_index, command in enumerate(checks):
        command_label = f"{label} candidateChecks[{check_index}]"
    ...
    for index, item in enumerate(consumers):
        ...
        parsed.append(
            FleetConsumer(
                ...
                candidate_checks=_parse_candidate_checks(item, label),
            )
        )
