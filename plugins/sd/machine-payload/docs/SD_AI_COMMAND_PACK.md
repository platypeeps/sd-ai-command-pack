...
- `~/.agents/bin/sd-ai-command-pack-full-check.sh`: canonical full-check script.
- `~/.agents/bin/sd-ai-command-pack-check.py`: schema-versioned read-only check
  coordinator.
- `~/.agents/bin/sd-ai-command-pack-review.py`: schema-versioned unified review
...
- `~/.agents/bin/sd-ai-command-pack-review-full-check.sh`: deterministic
  `sd-review-pr` selector for a repository-owned `check:full` wrapper or the
  canonical pack-script fallback.
- `~/.agents/bin/sd-ai-command-pack-shell-lib.sh`: shared Bash helpers sourced by
  the full-check, review-local, and review-scope scripts.
- `~/.agents/bin/sd-ai-command-pack-toolchain.sh`: non-mutating toolchain doctor and
  deterministic Python resolver used by SD workflows before dependency-sensitive
  checks.
- `~/.agents/bin/sd-ai-command-pack-audit-route.py`: deterministic repository
  fingerprinting and charter selection for standard and exhaustive audits.
- `~/.agents/bin/sd-ai-command-pack-audit-inventory.py`: read-only committed-tree
...
- `~/.agents/bin/sd-ai-command-pack-housekeeping.sh`: canonical post-merge housekeeping script.
- `~/.agents/bin/sd-ai-command-pack-housekeeping-result.py`: read-only composer for
  schema-versioned housekeeping action, eligibility, and final-status evidence.
- `~/.agents/bin/sd-ai-command-pack-pr-eligibility.py`: read-only exact-head
...
- `~/.agents/bin/sd-ai-command-pack-status.py`: read-only local/fleet status collector
  and schema-versioned JSON reporter used by housekeeping final verification.
- `~/.agents/bin/sd-ai-command-pack-work-loop.py`: standard-library user-local loop
  ledger, lock, focus ranking, transition, reconciliation, and resume helper.
- `~/.agents/bin/sd_ai_command_pack_fleet_lib.py`: shared fleet-manifest validation,
  machine-profile resolution, checkout override, and release-ledger contracts.
- `~/.agents/bin/sd-ai-command-pack-record-session.py`: one-shot session journal
...
- `~/.agents/bin/sd-ai-command-pack-install-audit.py`: structural post-install audit
  for missing installed targets and unlisted pack-like files.
- `~/.agents/bin/sd-ai-command-pack-recovery-artifacts.py`: receipt-driven lifecycle
  manager for pack-created Git recovery stashes and worktrees; described in
  detail in the recovery-artifacts section below.
- `~/.agents/bin/sd-ai-command-pack-surface-check.py`: schema-versioned shipped-surface
...
- `~/.agents/bin/sd-ai-command-pack-pr-body-scope.py`: configurable PR-body scope
  preflight for broad behavior-changing diffs.
- `~/.agents/bin/sd-ai-command-pack-update-spec-kb.py`: Obsidian KB copy-folder
  refresh helper for the update-spec workflow.
- `~/.agents/bin/sd-ai-command-pack-pack-update.sh`: the single machine update
...
bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python -- \
  ~/.agents/bin/sd-ai-command-pack-review-learnings.py --include-working-tree
```
...
`~/.agents/bin/sd-ai-command-pack-recovery-artifacts.py` owns the lifecycle of
...
`sd-check` runs `~/.agents/bin/sd-ai-command-pack-check.py` through the installed
...
`~/.agents/bin/sd-ai-command-pack-surface-check.py`. That schema-version-1 validator
...
`~/.agents/bin/sd-ai-command-pack-update-spec-kb.py` to maintain `.obsidian-kb/` in the
...
`python3 ~/.agents/bin/sd-ai-command-pack-update-spec-kb.py --dry-run` to preview the
...
`bash ~/.agents/bin/sd-ai-command-pack-housekeeping.sh --self-test`, which verifies
...
`~/.agents/bin/sd-ai-command-pack-audit-route.py` helper deterministically records
...
`~/.agents/bin/sd-ai-command-pack-audit-inventory.py` helper to inspect committed Git
...
```bash
bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python -- \
  ~/.agents/bin/sd-ai-command-pack-work-loop.py reconcile-terminal --repo . \
  --run-id <run-id> \
...
bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh doctor --json
bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python \
...
`bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run -- <tool> [args...]`; use that
...
- `SD_AI_COMMAND_PACK_FULL_CHECK_KB`: Obsidian KB freshness check mode.
  Default `auto` runs `~/.agents/bin/sd-ai-command-pack-update-spec-kb.py --check`
...
bash -n ~/.agents/bin/sd-ai-command-pack-full-check.sh
...
- `~/.agents/bin/sd-ai-command-pack-update-spec-kb.py` is missing: reinstall the pack;
...
- `~/.agents/bin/sd-ai-command-pack-full-check.sh` is missing: reinstall the pack; every target
  repo should receive the shared script.
- `~/.agents/bin/sd-ai-command-pack-housekeeping.sh` is missing: reinstall the pack; every
