...
5. Structural post-install audit through
   `~/.agents/bin/sd-ai-command-pack-install-audit.py`, verifying that copied pack
...
Common toggles are below. See `~/.agents/docs/SD_AI_COMMAND_PACK.md` → Configuration for
...
- In sandboxed agent sessions, set writable temp-backed caches before rerunning
  ...
  copy/paste block lives in `~/.agents/docs/SD_AI_COMMAND_PACK.md` → Configuration.
