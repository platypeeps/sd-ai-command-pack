...
1. Resolve the repository root and verify these installed sibling files exist:
   `~/.agents/bin/sd-ai-command-pack-toolchain.sh`,
   `~/.agents/bin/sd-ai-command-pack-check.py`, and
   `~/.agents/bin/sd_ai_command_pack_lib.py`. If any is missing, stop and recommend
