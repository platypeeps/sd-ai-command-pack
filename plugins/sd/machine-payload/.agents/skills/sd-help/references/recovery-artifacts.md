...
1. **Create**, then **register immediately.** Registration is atomic and
   ...
   ```bash
   artifact_id="$(bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python -- \
     ~/.agents/bin/sd-ai-command-pack-recovery-artifacts.py register \
...
3. **On the success path, retire in a `finally`.** After recovery succeeds, the
   ...
   bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python -- \
     ~/.agents/bin/sd-ai-command-pack-recovery-artifacts.py cleanup \
