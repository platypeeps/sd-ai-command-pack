...
- If the PR state is `MERGED` at startup, during polling, or before the final
  ...
  with `bash ~/.agents/bin/sd-ai-command-pack-housekeeping.sh`.
...
`bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python --require-module
...
bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python -- \
  ~/.agents/bin/sd-ai-command-pack-fleet-review-classify.py \
...
if [ "$PR_STATE" = "MERGED" ]; then
  bash ~/.agents/bin/sd-ai-command-pack-housekeeping.sh
  exit 0
fi
```

If `~/.agents/bin/sd-ai-command-pack-housekeeping.sh` is missing, read
...
```bash
bash ~/.agents/bin/sd-ai-command-pack-toolchain.sh run-python -- \
  ~/.agents/bin/sd-ai-command-pack-review-learnings.py \
...
3. Execute `sd-finish-work` as the primary instructions. That wrapper owns
   ...
   `~/.agents/bin/sd-ai-command-pack-record-session.py`; do not bypass it with a
