...
2. Verify that `sd-ai-command-pack-review-local.sh` is resolvable, either as a bare command on `PATH` or as a file at that path relative to the repository root. If the skill or script is missing, unreadable, empty, resolves to more than one candidate, fails validation, defines contradictory steps that violate this command's safety rules, requires unavailable tools, or cannot execute, stop and report the exact blocker.
