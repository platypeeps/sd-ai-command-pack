...
- If the PR state is `MERGED` at startup, during polling, or before the final
  ...
  with `bash sd-ai-command-pack-housekeeping.sh`.
...
If `sd-ai-command-pack-housekeeping.sh` is missing, read
...
3. Execute `sd-finish-work` as the primary instructions. That wrapper owns
   ...
   `sd-ai-command-pack-record-session.py`; do not bypass it with a
