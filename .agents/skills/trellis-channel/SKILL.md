...
- **Dispatcher wait pattern**: use `--kind done` / `--kind turn_finished` (trellis-emitted system events), not user-authored labels or prose as completion signals. LLM workers commonly write signal text into prose instead of emitting a machine-readable event. See `references/command-reference.md` "tag vs kind".
