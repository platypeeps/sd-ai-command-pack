...
(`bin/sd-docs-lint:454`) exempts a reference by its **shape** — a token holding
...
contain one". Rule 6's `LOG_HEADING_RE` (`bin/sd-docs-lint:327`) exempts by
**region**: `item_citations` (`bin/sd-docs-lint:330`) stops reading at a `Log`
...
(`bin/sd-docs-lint:298`) reads `.md` targets only, and rule 7 reads
...
(`bin/sd-docs-lint:396`) checks only items that carry a `.citations.tsv`, and
