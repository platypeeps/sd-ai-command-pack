...
| `classify()` | 32 | `check_work_references` (`bin/sd-docs-lint:457`) is 41 for walk-a-corpus, skip-with-a-named-reason, count, report; this is the same job without its git call and without `METAVARIABLE_RE`'s branch |
| `anchored_citations()` as a thin filter over it | 5 | it is 23 today and becomes a comprehension the existing assertion calls |
| `census()` | 10 | the accumulate-and-note half of `check_citations` (`bin/sd-docs-lint:396`), which is about 10 of its 52 |
...
| the marker regexes and `marker_after()` | 17 | `anchor_line` (`bin/sd-docs-lint:361`) is 15 for a small positional read whose docstring carries the reason |
