...
- [ ] **Step 1 — the question-form rule in `sd-grill`.** One file. Add the
      ...
      red-flags row, and record the parents' disagreement and its ruling in
      Lineage. Independently landable: it corrects a shipped skill and depends
...
- [ ] **Step 2 — `sd-debug`.** One new `skills/sd-debug/SKILL.md`. The
      ...
      `sd-postmortem`, `sd-check`, `sd-review`, `sd-red-team` and the `sd-rust-*`
      family. Independently landable and green on its own.
...
- [ ] **Step 3 — `sd-receive-review`, and the pointer back from
      ...
      and the bound on outward action. Plus one line in
      `skills/sd-feedback/SKILL.md` naming this skill as the owner of the job it
      already disclaims. Landable independently of steps 1 and 2; the
...
**Budget.** `git diff --stat origin/main...HEAD -- bin/ dashboard/` prints
nothing on the pushed branch. This item adds no Python, so any output at all
...
gated behind early access on this account. Requirements 5 through 8 are
therefore reader-verified, which is stated in the PRD rather than papered over,
and this item leaves the pack with three skills in that condition instead of
one.
