...
- [x] **3. The plugin loader retires, and the manifest loses two keys.** System
      ...
      **The plugin half of `app.js` is `drawPlugins` and `panelId`. It is not
      ...
      | Function | Step | Why |
      |---|---|---|
      | `drawPlugins` (in `dashboard/app.js`) | **3** | It polls `/api/plugins`, which this step's `server.py` edit removes. Delete it *with all three of its call sites* — `drawPlugins()` at `dashboard/app.js:780` [absent: step 3 shipped and removed drawPlugins with all three of its call sites; nothing in dashboard/app.js names the symbol today], `setInterval(drawPlugins, 10000)` at `:784`, and its entry in the redraw list at `:828` — or the page polls a deleted endpoint every ten seconds. |
