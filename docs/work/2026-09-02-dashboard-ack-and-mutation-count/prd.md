...
`do_POST` (`dashboard/server.py:435`) writes no record of having run — it validates, dispatches to
...
(`dashboard/server.py:522`) treats an empty list as an ordinary start, because loopback bound and
...
| Fact | Number | Source |
|---|---|---|
| `dashboard/` total lines | **4,159 against a 4,300 cap — 141 headroom** | `wc -l dashboard/*.py dashboard/*.js` |
| `dashboard/` code lines | **2,200 against a 2,300 cap — 100 headroom** | the tokeniser in `tests/test_loc_caps.py:115-134` |
| `RUN_ALLOWLIST` entries | 1 (`index`) | `dashboard/actions.py:52-58` |
| Mutating requests recorded | 0 — no write site exists | `dashboard/server.py:435-471` |
...
`DASHBOARD_CODE_CAP` (`tests/test_loc_caps.py:66`) moves **downward only**, so 100 lines is a
...
8. Everything that must live under `dashboard/` fits in its 100 lines of code, or the item
   explicitly proposes what to delete to make room. Raising `DASHBOARD_CODE_CAP` is not available.

   It was 110 when this item opened. The assigned-only change to the tracker views spent ten of
   them — a net spend, after deleting two tables and their renderer — and the number is restated
   here rather than left at the figure the budget below was first divided against.
   ...
   this item's first mechanism, leaving ~41 for an ack store and its control. `dashboard/app.js`
...
- **C-7 — the observed loopback-only start's cause was inferred, not seen. Material. `addressed` by
  ...
  `bound_addrs` (`dashboard/server.py:125`) latches it, and `serve` (`dashboard/server.py:522`)
