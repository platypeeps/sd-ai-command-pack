...
| Fact | Number | Source |
|---|---|---|
| `dashboard/` total lines | **4,171 against a 4,300 cap — 129 headroom** | `wc -l dashboard/*.py dashboard/*.js` |
| `dashboard/` code lines | **2,201 against a 2,300 cap — 99 headroom** | the tokeniser in `tests/test_loc_caps.py:115-134` |
...
`DASHBOARD_CODE_CAP` (`tests/test_loc_caps.py:66`) moves **downward only**, so 99 lines is a
...
8. Everything that must live under `dashboard/` fits in its 99 lines of code, or the item
   explicitly proposes what to delete to make room. Raising `DASHBOARD_CODE_CAP` is not available.

   It was 110 when this item opened. The assigned-only change to the tracker views spent eleven of
   ...
   this item's first mechanism, leaving ~40 for an ack store and its control. `dashboard/app.js`
