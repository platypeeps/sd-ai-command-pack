...
`do_POST` (`dashboard/server.py:498`) writes no record of having run — it validates, dispatches to
...
(`dashboard/server.py:621`) treats an empty list as an ordinary start, because loopback bound and
...
| Mutating requests recorded | 0 — no write site exists | `do_POST` (`dashboard/server.py:498-570`) |
...
- **C-7 — the observed loopback-only start's cause was inferred, not seen. Material. `addressed` by
  ...
  `bound_addrs` (`dashboard/server.py:129`) latches it, and `serve` (`dashboard/server.py:621`)
