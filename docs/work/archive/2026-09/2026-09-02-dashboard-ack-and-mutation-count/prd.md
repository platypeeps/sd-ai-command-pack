...
`do_POST` (`dashboard/server.py:516`) writes no record of having run — it validates, dispatches to
...
(`dashboard/server.py:639`) treats an empty list as an ordinary start, because loopback bound and
...
| Fact | Number | Source |
|---|---|---|
| `dashboard/` total lines | **4,190 against the 4,300 cap in force when this was measured; R11-D29 re-derived it at 4,350** | the tokeniser `line_count` (`tests/test_loc_caps.py:181-189`), over `tracked("dashboard")` |
| `dashboard/` code lines | **2,206 against a 2,300 cap — 94 headroom** | the tokeniser `code_line_count` (`tests/test_loc_caps.py:135-178`) |
| `RUN_ALLOWLIST` entries | 1 (`index`) | `dashboard/actions.py:52-58` |
| Mutating requests recorded | 0 — no write site exists | `do_POST` (`dashboard/server.py:516-588`) |
...
`DASHBOARD_CODE_CAP` (`tests/test_loc_caps.py:109`) moves **downward only**, so 94 lines is a
...
`test_the_dashboard_stays_under_its_ceiling` (`tests/test_loc_caps.py:233-244`) —
...
8. Everything that must live under `dashboard/` fits in its 94 lines of code, or the item
   ...
   2026-09-03 (1,584 after this item). A
...
- **C-7 — the observed loopback-only start's cause was inferred, not seen. Material. `addressed` by
  ...
  `bound_addrs` (`dashboard/server.py:129`) latches it, and `serve` (`dashboard/server.py:639`)
