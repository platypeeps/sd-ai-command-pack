---
title: "Retire sd-review-pr: port the fleet integration-only review profile into sd-review"
status: planning
...
> **Split into three ordered children (2026-08-21).** This task is now a
> coordination parent and must not be implemented directly. Its eight
> requirements (R1-R8) and eight acceptance criteria were distributed as:
>
> R-numbers in this table are **this parent's** R1-R8, listed under
> "Requirements" below. Each child renumbers its own requirements from R1 and
> adds requirements this parent never had, so child R-numbers do **not**
> correspond to the numbers in this column. Cite by description, not by number,
> when crossing between parent and child.
>
> | Order | Child | Owns (parent R-numbers) | Child's own range |
> | --- | --- | --- | --- |
> | 1 | `08-21-port-integration-only-profile` | R1, R2, R3 — profile port, fleet repoint, recheck relocation | R1-R5 |
> | 2 | `08-21-delete-review-pr-surface` | R4, R5 (commands), R7, R8 — surface deletion, registry row, closure allowlist | R1-R6 |
> | 3 | `08-21-retire-full-check-family` | R6, R5 (scripts) — gate relocation, `make check` recomposition, script/test/env deletion | R1-R7 |
>
> Ordering is written into each child's own PRD, not implied by tree position.
> Child 2 blocks on child 1. Child 3 is correctness-independent of both and is
> ordered last only to serialize `RETIRED_TARGETS` edits.
>
> **Ceded scope.** `review-full-check.sh` and its `templates/` twin are no
> longer in this tree — ownership stays with
> `08-08-phase0-dead-surface-cleanup`, which already scopes that deletion.
> Separately, `07-25-reduce-review-tooling-spawns` R1/R2/R4 (memoizing base-ref
> discovery *inside* `full-check.sh`) become resolved-by-removal when child 3
> lands; that task's `review-preflight.mjs` scope is unaffected.
>
> **Citation drift.** The line references in "Why this is a separate task" and
> "Scope" below were written 2026-08-09 and had drifted by up to 118 lines.
> Ten were re-verified and corrected on 2026-08-21; the per-profile behavior
> citations (`:112`, `:196-209`, `:226`, `:251`, `:273`, `:345`, `:409`,
> `:486`, `:641`, `:663`, `:675-677`) were **not** re-verified and should be
> treated as approximate. Each child PRD carries its own re-verified citations;
> prefer those.
...
- `sd-review-pr/SKILL.md` implements that contract across `:69-77` (trusted
...
3. **Relocate the Fleet Integration-Only Recheck procedure** (R9 of the parent
   PRD) from `templates/.agents/skills/sd-review-pr/SKILL.md:206-232` into the
   source-only `sd-fleet-refresh` skill before deletion. It invokes
   `fleet-review-classify.py`, which `install-audit.py:121` marks source-only,
...
4. **Delete the `sd-review-pr` surface**: 11 source files (verified 2026-08-21), 24 manifest rows, the
   2 short-name command rows, and the generated mirrors.
5. **Delete the two full-check scripts** —
   `sd-ai-command-pack-full-check.sh` (1123 lines) and
   ...
   **Corrected 2026-08-09:** these are *not* reachable from `sd-review-pr` —
   `sd-review-pr/SKILL.md:295-297` explicitly forbids falling back to either.
   They land in this task for two independent reasons: `full-check.sh` is the
   repo's own gate (`Makefile:123-126`, `.github/workflows/tests.yml:665`),
...
6. **Everything that deletion then unblocks** — this is the bulk of the work
   ...
   - **Relocate `run_pack_source_drift_gates`.** It is a *function inside*
     `full-check.sh`, and `.github/workflows/tests.yml:665` runs
   ...
   - **The `make check` composition** (parent design option A):
     `Makefile:123-126` is `full-check:` plus `check: test lint audit
   ...
   - **Remove `SD_AI_COMMAND_PACK_FULL_CHECK_PRISM=0 ..._GITO=0`**
     (`Makefile:124`) together with the lanes it disables — inseparable, because
   ...
   - **`tests/test_full_check.py`** (1633 lines) and
