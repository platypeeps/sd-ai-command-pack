---
title: "PARKED: The developer identity file is gitignored, so every fresh worktree fails record-session"
status: done
created: 2026-08-08
---
...
`task.py start`, which sets `in_progress` locally, but nothing here was
implemented, so the committed record stays `planning`. That matches every other
parked task in this repository and it is what the finalization validator
requires: a planning-mode bundle needs `status` `planning`, `completedAt` null,
and `branch` null both in the bundle and at its base
(`sd-ai-command-pack-review-preflight.mjs:2532` and `:2544`), and CI applies the
same check to each pushed bookkeeping range. Publishing `in_progress` and
correcting it in a later commit makes the intermediate push unvalidatable, which
is worth knowing before parking a started task. A resume runs `task.py start`
again.
...
into docs/work. Recover the bodies from git history under `.trellis/tasks/archive/2026-08/08-08-developer-identity-not-in-worktrees`:

- research/staged_test_worktree_identity.py
