---
title: Verify remote reviewer requests actually materialize
status: done
created: 2026-07-09
---
...
- [x] The default CLI trigger is exactly `gh pr edit <pr> --add-reviewer @copilot`.
- [x] The bare `copilot-pull-request-reviewer` slug is absent from active
      defaults and request examples.
- [x] Copilot author matching defaults to
      `copilot-pull-request-reviewer[bot]`.
- [x] Tests cover default Copilot, generic reviewer, custom request command,
      and optional explicit REST Copilot behavior.
- [x] Tests cover a successful request command with no review activity and
      prove that it is not counted as a completed round.
- [x] Tests cover late-arriving inline comments within the settle window.
- [x] User-facing diagnostics distinguish policy/availability failures from
      no-materialization after an accepted request.
- [x] Root/template parity, install audit, KB freshness, and deterministic
      full-check pass.
