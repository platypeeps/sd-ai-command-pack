---
title: Test the sd-check read-only git guard
status: done
created: 2026-07-28
---
...
- Also cover the currently uncovered `perl -e` branch at `scripts/sd-ai-command-pack-check.py:501`.
- Raise the per-file coverage floor for `scripts/sd-ai-command-pack-check.py` in `.github/scripts/check-shipped-script-coverage.sh:33` to at or just below the new measured value.
...
- The floor for `scripts/sd-ai-command-pack-check.py` is 70% against 73% actual, so deleting the guard currently lands green.
