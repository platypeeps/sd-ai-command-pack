...
- Add pre-archive and completion/planning final-bundle good/base/failure
  fixtures, including the blank-description failure from the recent main push
  and a valid planning-task-plus-journal bundle with no archive move.
...
- Run the mode-specific `final-bundle` after completion or planning journal
  creation and before any push.
...
- Prove valid finalization retains commit order, one push, and exact-head
  downstream gates.
- Prove planning mode rejects every archive/completion/pointer mutation while
  completion mode still requires its archive move.
