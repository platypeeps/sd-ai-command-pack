---
title: Fix documentation and help-text accuracy batch
status: done
created: 2026-07-09
---
...
- R7: Record the `FIRST_REPLY_NOTICE` ownership decision. These hooks are
  Trellis-owned generated runtime, so do not edit or review them in this pack;
  preserve a paste-ready upstream handoff instead.

## Acceptance Criteria

- [x] `python3 install.py --help` names all three preserved files and the
      `--codex` flag; a parity/doc test pins the preserved-file list against
      `FORCE_PRESERVED_TARGETS` so it cannot drift again.
- [x] No spec file cites `install.py` for a symbol that lives in `installer/*`.
- [x] AGENTS.md routes an agent to CONTRIBUTING.md / `make check` / specs.
- [x] SECURITY.md present.
- [x] README + guide enumerations complete; docs twins byte-identical;
      review-preflight doc-path checker still passes.
- [x] FIRST_REPLY_NOTICE ownership decision recorded without changing
      Trellis-owned generated runtime.

## FIRST_REPLY_NOTICE Ownership Decision (2026-07-14)

The notice is defined by upstream Trellis in its Codex and shared session-hook
templates. The pack manifest does not install or own those hook files. Changing
the copies in this repository would be overwritten by `trellis update` and
would violate the maintainer rule against reviewing or modifying original
Trellis payload. This task therefore leaves those files untouched.

Paste-ready upstream handoff:

```markdown
Task: decide and normalize Trellis FIRST_REPLY_NOTICE behavior.

Repo: mindfold-ai/Trellis

The Codex and shared session-start hook templates seed a hardcoded
locale-specific FIRST_REPLY_NOTICE, while the Copilot session-start template
does not. Please decide whether this is intended end-user behavior. If it is,
gate it on an explicit locale/config signal and apply the contract consistently
across supported session-hook platforms. If it is not, remove it from the
canonical Trellis templates. Add focused generation/runtime tests. Consumer
repos and sd-ai-command-pack must not patch these Trellis-owned generated files.
```
