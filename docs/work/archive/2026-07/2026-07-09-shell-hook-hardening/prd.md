---
title: Shell and hook hardening batch
status: done
created: 2026-07-09
---
...
- [x] Rename-into-chore push is rejected by the guard (test in R2).
- [x] pre-push behavioral tests added and green on the CI matrix.
- [x] `full-check.sh` leaves no temp files after Ctrl-C mid-run (manual
      verification note acceptable) and its traps are covered where testable.
- [x] shell-lib header lists all required globals; a from-scratch caller
      following it runs without `command not found`/empty-arithmetic errors.
- [x] Housekeeping self-test (`--self-test`) extended to a scenario with an
      empty TSV field and an empty DEFAULT_BRANCH; both handled safely.
- [x] shellcheck `-S warning` stays clean; scripts/templates twins
      byte-identical.
