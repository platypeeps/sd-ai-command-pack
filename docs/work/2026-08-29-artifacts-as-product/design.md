...
`sd plugin sync` copy machinery). **sd-writing-pack is the first plugin**: pack.py 2,532 → **1,590**
...
`content/<year>/<slug>/`, which `store.driver = vault` cannot reach at all; `main` is 93, carrying
the whole argparse tree; and 448 are imports, constants and module-level code. ~1,250 was never
...
Step 10 closed it: `tips attach` and `tips render` read a tip through `sd store get --json`
instead of by path, `topics seed` was deleted, and with them went the last `VAULT`-derived
constant -- `pack.py` no longer knows where the vault is.
...
(`dashboard/app.js:481`) composes the DOM id from the plugin's own prefix and tab name, lowercased
