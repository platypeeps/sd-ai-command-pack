...
- [x] 3 — new installer + 12 skills + sd-review backends. **Split into reviewable sub-PRs
  ...
  **Closed 2026-08-30, on the row's own check and not a proxy for it.** The table row gates
  ...
  - [x] 3e — machine-scope installer + `installed.json` + parity tests; deletes the legacy render
        ...
        atomic on merge. **365 files, 183,446 deletions against 1,343 insertions.**
...
- [x] P4 — 2026-08-30. The nightly `skill-proposal-accept` routine files its one
  ...
  - **Vault-side, in one commit** (`b6bd446`, six enumerated paths): `System/Scripts/
...
- [ ] 6b — dashboard swap to :8767 behind the parity checklist.
  ...
  - **6b-2 landed: the plugin loader, and the dashboard cap now shows the
    ...
    **The measurement:** `dashboard/` is **2,054 of 2,500, 446 left**. The
    loader cost 555 against the ~240 R11-D13 left for the loader *and*
    `RUN_ALLOWLIST` together. R11-D13's backbone-side lift is 763; 763 into 446
    does not fit, so `dashboard/` projects to ~2,817 with `RUN_ALLOWLIST` still
    uncounted. Not raised here -- the test passes at 2,054, and the cap gets
