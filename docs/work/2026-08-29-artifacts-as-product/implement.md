...
| **3-c** | Consumer PRs, **one per repo**, removal only (M2): trellis payload, router removal via `--remove-legacy`, dotfiles, path rewrites, across 8 repos — the ninth, `mezmo_benchmark`, is frozen under D7 | per repo: zero trellis/router greps; CI green |
