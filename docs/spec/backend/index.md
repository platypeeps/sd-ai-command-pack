...
|-------|-----------------|
| [Manifest And Filesystem](./manifest-and-filesystem.md) | The manifest/installer/plugin-generation/payload-gate/fleet-campaign model. Partly stale: its Machine-Scope Installer section is a design record for files that no longer exist, and its gitignore-block section is removed, so `.gitignore` carries no markers |
