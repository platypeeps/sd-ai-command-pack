...
`bash templates/scripts/sd-ai-command-pack-toolchain.sh run -- <tool> [args...]`. A
...
- Applicability preflight:
  ...
  templates/scripts/sd-ai-command-pack-audit-route.py --repo . --mode
...
- Durable helper: `templates/scripts/sd-ai-command-pack-work-loop.py` owns the user-local
...
- Follow-up owner helper:
  `templates/scripts/sd-ai-command-pack-update-spec-kb.py --if-present`.
...
- Freshness and refresh helper:
  `python3 templates/scripts/sd-ai-command-pack-update-spec-kb.py [--check]`.
...
python3 templates/scripts/sd-ai-command-pack-update-spec-kb.py --check || {
  ...
  python3 templates/scripts/sd-ai-command-pack-update-spec-kb.py
  python3 templates/scripts/sd-ai-command-pack-update-spec-kb.py --check
}
```
...
- Shipped-surface validation entry point:
  `templates/scripts/sd-ai-command-pack-surface-check.py [--json] [--base-ref REF]`.
...
- Repository configuration: `.sd-ai-command-pack/check.json` [absent: target-repo install path], schema version 1.
...
- Coordinator: `python3 templates/scripts/sd-ai-command-pack-review.py --repo PATH
...
`templates/scripts/sd-ai-command-pack-full-check.sh` is a pack-source release gate with
...
`templates/scripts/sd-ai-command-pack-review-preflight.mjs` on the complete branch plus
...
`_SEED_EXAMPLE` text (`.trellis/scripts/common/task_store.py:139` [absent: target-repo Trellis path]). That string
...
- `sd-finish-work` -> resolved `trellis-finish-work` skill plus
  `templates/scripts/sd-ai-command-pack-record-session.py` at journal time and
...
`trellis-update-spec` skill, follow that skill as-is for its `docs/spec/`
...
- Search for an existing architecture overview, such as `ARCHITECTURE.md`,
  `docs/ARCHITECTURE.md`, or `docs/spec/**/architecture*.md`.
...
- Run `python3 templates/scripts/sd-ai-command-pack-update-spec-kb.py` to create or
  ...
  `docs/spec/**/*.md`, `.trellis/workflow.md` [absent: target-repo Trellis path], `.trellis/config.yaml` [absent: target-repo Trellis path], and
