...
PRs. The fix added an explicit exact-head post-finish attestation at the shell
boundary and a repository-owned regression outside the installed pack
footprint.
...
**Real-world example**: Agent-session update hints fetched npm `latest` metadata with `response.read(4096)` and then parsed it as complete JSON. The upstream package's metadata exceeded 4 KB, so the JSON was truncated, parse failed silently, and the first session injection showed no update hint. Fix: read the complete response before parsing, and add a regression where `version` is followed by an 8 KB metadata tail.
